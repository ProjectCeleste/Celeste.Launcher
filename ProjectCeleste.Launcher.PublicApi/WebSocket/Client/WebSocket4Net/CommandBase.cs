...
{
    internal class CommandBase<TRequest, TResponse> where TRequest : IGenericRequest where TResponse : IGenericResponse
    {
        ...
        public CommandBase(Client webSocketClient, string commandName, bool preventSpam = false,
        ...
        public async Task<TResponse> Execute(TRequest request)
        {
            ...
            var result = await DoDataExchange(_webSocketClient, request, CommandName);
        ...
        {
            using var responseAction = new AwaitableResponse<TResponse>();
