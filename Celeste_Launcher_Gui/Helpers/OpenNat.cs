...
namespace Celeste_Launcher_Gui.Helpers
{
    public static class OpenNat
    {
        ...
        public static async Task<int> MapPortTask(int privatePort, int publicPort)
        {
            ...
            var localIp = Array.Find(Dns.GetHostEntry(Dns.GetHostName()).AddressList, key => key.AddressFamily == AddressFamily.InterNetwork);
            ...
            if (enumerable.Any(key => //Equals(key.PrivateIP, localIp) && key.Description == "AOEO Project Celeste" 
                key.PublicPort == 1000))
            {
                var r = Array.Find(enumerable, key => //Equals(key.PrivateIP, localIp) && key.Description == "AOEO Project Celeste" 
                        key.PublicPort == 1000);
